...
#include <chrono> // Required for timer literals like 5s
...
dlio::MapNode::MapNode() : Node("dlio_map_node") {
  ...
  // Service creation syntax update
  ...
  // FIX: ROS 2 Wall Timer initialization
  this->map_timer = this->create_wall_timer(
      5000ms, std::bind(&dlio::MapNode::publishMap, this));
...
// FIX: Removed TimerEvent argument
void dlio::MapNode::publishMap() {
  if (this->dlio_map->points.empty()) return;

  sensor_msgs::msg::PointCloud2 map_ros; // Ensure msg namespace is used
  pcl::toROSMsg(*this->dlio_map, map_ros);
  
  // FIX: ROS 2 uses this->now()
